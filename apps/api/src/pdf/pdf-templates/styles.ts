...
.title-box .title-subtitle { font-size: 8.5pt; font-weight: 400; color: ${primaryColor}; opacity: 0.78; margin-top: 2px; letter-spacing: 0.3px; }

/* ต้นฉบับ / สำเนา stamp — positioned absolutely top-right of body padding. */
.oc-badge {
  position: absolute; top: 14mm; right: 14mm;
  padding: 4px 14px; border: 1.5px solid ${primaryColor}; border-radius: 4px;
  font-size: 10pt; font-weight: 700; color: ${primaryColor};
  transform: rotate(-8deg); transform-origin: right top;
  background: rgba(255,255,255,0.85);
  letter-spacing: 1px;
}
.oc-badge.badge-copy { color: #6b6b6b; border-color: #6b6b6b; }
...
table.items tbody td { padding: 3.5px 6px; border: 1px solid #c8c8c8; vertical-align: top; }
...
.summary-grid { margin-top: 8px; display: grid; grid-template-columns: 1fr 240px; gap: 0; }
...
.notes { margin-top: 8px; font-size: 8.5pt; color: #444; }
...
.signatures { margin-top: 10px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; font-size: 9pt; }
.sig-box { border-top: 1px dotted transparent; padding-top: 18px; }
