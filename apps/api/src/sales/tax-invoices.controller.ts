...
const ENTITY = 'SalesDocument:TAX_INVOICE';

@Controller('sales/tax-invoices')
@UseGuards(JwtAuthGuard, RolesGuard)
export class TaxInvoicesController {
  constructor(private taxInvoices: TaxInvoicesService) {}

  @Get()
  list(@CurrentUser() user: AuthUser, @Query() dto: ListSalesDocumentsDto) {
    return this.taxInvoices.list(user.companyId, dto);
  }

  @Get(':id')
  get(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return this.taxInvoices.findOne(user.companyId, id);
  }
  ...
  @AuditAction('CREATE_DOCUMENT', {
    ...
    getMetadata: (_req, res) => ({ type: 'TAX_INVOICE', number: res?.number, grandTotal: res?.grandTotal }),
  })
  create(@CurrentUser() user: AuthUser, @Body() dto: CreateSalesDocumentDto) {
    return this.taxInvoices.create(user.companyId, user.id, dto);
  }
  ...
  confirm(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return this.taxInvoices.confirm(user.companyId, user.id, user.role, id);
  }
  ...
  void(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: VoidDocumentDto) {
    return this.taxInvoices.void(user.companyId, user.id, user.role, id, dto.reason);
  }
}
