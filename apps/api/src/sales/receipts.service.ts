...
export class ReceiptsService {
  ...
  create(companyId: string, userId: string, dto: CreateSalesDocumentDto) {
    return this.salesDoc.create('RECEIPT', companyId, userId, dto);
  }
  ...
  findOne(companyId: string, id: string) {
    return this.salesDoc.findOne('RECEIPT', companyId, id);
  }

  list(companyId: string, query: ListSalesDocumentsDto) {
    return this.salesDoc.list('RECEIPT', companyId, query);
  }
}
